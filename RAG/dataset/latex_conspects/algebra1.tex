...
\begin{itemize}
    \item $\mathbb{Z}$ - целые числа $+$ $-$ $\cdot$ $>$
    \item $\mathbb{N}$ - натуральные числа
    \item $\mathbb{R}$ - вещественные числа
\end{itemize}

\textbf{Аксиома индукции.} $A \subset \mathbb{N}; A \neq \emptyset \Rightarrow$ в $A$ есть наименьший элемент
...
\textbf{Def.} $a, b \in \mathbb{Z}, a \, \vdots \, b (b | a)$, если $\exists c \in \mathbb{Z}: a = bc$

\textbf{Rem.} $0 \, \vdots \, 0$ $\forall x \in \mathbb{Z} 0 = 0 \cdot x$

\textbf{Основные свойства делимости:}
\begin{enumerate}
    \item $0 \, \vdots \, a$
    \item $a \, \vdots \, 1$
    \item $a, b \, \vdots \, c \Rightarrow a + b \, \vdots \, c$
    \item $a, k \, \vdots \, c \Rightarrow k \cdot a \, \vdots \, c$
    \item $a \, \vdots \, a$
    \item $a \, \vdots \, b, b \, \vdots \, a \Rightarrow a = \pm b$
...
\begin{enumerate}
    \item[3.] $a \, \vdots \, c \Rightarrow \exists q_a: a = q_a \cdot c$
...
\textbf{Задача:} при каких $a, b, c \in \mathbb{Z}$ уравнение $ax + by = c$ имеет решение в целых числах ($\Leftrightarrow$ из чего состоит $<a, b>$? $c \in <a, b>$?)

\textbf{Def.} Идеалом называется подмножество $I \subset \mathbb{Z}$:
\begin{enumerate}
    \item $I \neq \emptyset$
    \item $a, b \in I \Rightarrow a + b \in I$
    \item $a \in I, k \in \mathbb{Z} \Rightarrow a \cdot k \in I$
\end{enumerate}

\textbf{Ex. 1} $c \in \mathbb{Z}$

$<c> = \{n \cdot c\} = \{ x \in \mathbb{Z} | x \, \vdots \, c\}$ - идеал, порожденный $c$ - главный идеал

\textbf{Ex. 2} $c_1, c_2 \cdots c_k \in \mathbb{Z}$

$<c_1, c_2 \cdots c_k> = \{n_1 c_1 + n_2 c_2 + \cdots + n_k c_k | n_i \in \mathbb{Z} \}$

{\textbf{Th.} в $\mathbb{Z}$ любой идеал - главный}
...
\begin{enumerate}
    \item $I = \{0\} = <0>$
    \item $\exists a \in I, a \neq 0 \Rightarrow a \in I, a \in \mathbb{N}$. Рассмотрим наименььший натуральный $b \in I$
    ...
    $r \in \mathbb{N}$ - противоречие ($b$ - наименььшее) $\Rightarrow r \notin \mathbb{N} \Rightarrow r = 0$
\end{enumerate}

В частности $\forall a, b \in \mathbb{Z}$ $\exists d: <a, b> = <d>$

\textbf{Def.} $a, b \in \mathbb{Z}$ НОД$(a, b) = gcd(a, b) = (a, b)$ - такое $d \in \mathbb{Z}$, что:
\begin{enumerate}
    \item $a \, \vdots \, d, b \, \vdots \, d$
    \item $\forall d': a \, \vdots \, d', b \, \vdots \, d' \Rightarrow d \, \vdots \, d'$
\end{enumerate}
...
\textbf{Th.} $a, b \in \mathbb{Z}$

\begin{enumerate}
    \item $\exists (a, b) = d$
    \item $\exists x, y \in \mathbb{Z}: ax + by = d$ - линейное представление НОДа
    \item $ax + by = c$ имеет решение $\Leftrightarrow c \, \vdots \, d$
\end{enumerate}
...
\begin{itemize}
    \item[$\Rightarrow$:] $c = ax + by \begin{cases} a \, \vdots \, (a, b) \\ b \, \vdots \, (a, b) \end{cases} \Rightarrow c = ax + by \, \vdots \, (a, b)$
    \item[$\Leftarrow$:] Пусть $c \, \vdots \, (a, b) = d$, т.е. $c = d \cdot k, k \in \mathbb{Z}$
...
\begin{enumerate}
    ...
    \item $(a, b) = (r_1, b) = (r_1, r_2) \cdots$, $\exists i \in \mathbb{N}: r_i = 0$
    \item $(a, b) = \cdots = (r_k, r_k+1) = (r_k, 0) = r_k$
\end{enumerate}

\textbf{Rem.} $a_1, a_2 \cdots a_k \in \mathbb{Z}$

$\exists (a_1, a_2 \cdots a_k) = d$ $\exists x_1 \cdots x_k \in \mathbb{Z}:$
...
\textbf{Def.} $a, b \in \mathbb{Z}$ называются взаимнопростыми, если $(a, b) = 1$
...
\begin{itemize}
    ...
    \item[$\Leftarrow$] $ax + by = 1 \Rightarrow 1 \, \vdots \, (a, b)$ $(a, b) = 1$
\end{itemize}

\textbf{Lm.} об отбрасывании взаимнопростого множителя

$a, b, c \in \mathbb{Z}$ $\begin{cases} ab \, \vdots \, c \\ (a, c) = 1 \end{cases} \Rightarrow b \, \vdots \, c$
...
\textbf{Def.} $p \in \mathbb{Z}$. $p$ называется простым, если
...
\textbf{Lm.} $p$ - простое $\Leftrightarrow ab \, \vdots \, p \Rightarrow \left [ \begin{gathered} a \, \vdots \, p \\ b \, \vdots \, p \end{gathered} \right. , |p| > 1$
...
\begin{itemize}
    \item[$\Leftarrow$] $p = xy \Rightarrow xy \, \vdots \, p \Rightarrow \left [ \begin{gathered} x \, \vdots \, p \\ y \, \vdots \, p \end{gathered} \right. \Rightarrow \left [ \begin{gathered} |x| \geq p \\ |y| \geq p \end{gathered} \right.$ 

    \item[$\Rightarrow$] Пусть $p$ - простое, $ab \, \vdots \, p$
...
Будем доказывать единственность и существование разложения $n = p_1 p_2 \cdots p_s, p_i$ - простые, $n \in \mathbb{N}$
...
\textbf{Rem.} $v_p(a)$ - это такое $n$, что $a \, \vdots \, p_n, a \not\, \vdots \, p^{n+1}$
...
\begin{enumerate}
    ...
    $n \neq m$ $p^{n-m} x \, \vdots \, p \Rightarrow p^{n-m} x + y \not\, \vdots \, p \Rightarrow p^m(p^{n-m} x + y) \not\, \vdots \, p^{m+1}$
...
Утверждение: $a = \prod\limits_{p_i \in \mathbb{P}} p_i^{a_i}$, $b = \prod\limits_{p_i \in \mathbb{P}} p_i^{b_i}$

Тогда 

\begin{enumerate}
    \item $a \, \vdots \, b \Leftrightarrow a_i \geq b_i \forall i$
    \item $\exists c : a = c^k \Leftrightarrow a_i \, \vdots \, k \forall i$
...
\begin{enumerate}
    \item $c \, \vdots \, a, c \, \vdots \, b$
    \item $c' \, \vdots \, a, c' \, \vdots \, b \Rightarrow c' \, \vdots \, c$
\end{enumerate}
...
\begin{enumerate}
    ...
    $\begin{gathered} \prod p_i^{a_i} \\ \prod p_i^{a_i} \end{gathered} \, \vdots \, \prod p_i^{min(a_i, b_i)}$, т.е. $a, b \, \vdots \, \prod p_i^{min(a_i, b_i)}$

    $a, b \, \vdots \, \prod p_i^{c_i}$ $\forall i$ $\begin{gathered} c_i \leq a_i \\ c_i \leq b_i \end{gathered} \Rightarrow c_i \leq min(a_i, b_i) \Rightarrow \prod p_i^{min(a_i, b_i)} \, \vdots \, \prod p_i^{c_i}$
...
\textbf{Def.} $a, b, n \in \mathbb{Z}$ $a$ сравнимо с $b$ по модулю $n$, если $(a-b) \, \vdots \, n$ обозначается $a \equiv b (mod n) \Rightarrow \mathbb{Z}$ разбивается на классы эквивалентности. Обозначение: $R$ -- отношение, $M/R$ -- множество классов эквивалентности, $\sim$ -- эквивалентность $M/\sim$ -- множество классов эквивалентности -- фактормножество
...
\begin{itemize}
    \item[Р:] $a - a = 0 \, \vdots \, n \Rightarrow a = a$
    \item[С:] $a \equiv b \Rightarrow a - b \, \vdots \, n \Rightarrow b - a \, \vdots \, n \Rightarrow b \equiv a$
    \item[Т:] $\begin{cases}
        a \equiv b \\
        b \equiv c
...
\begin{enumerate}
    \item $(a + c) - (b + d) = (a - b) + (c - d) \, \vdots \, n$
    \item $ac \equiv bc$, т.к. $ac - bc = c(a - b) \, \vdots \, n$
...
\textbf{Th.} ($\mathbb{Z}/n\mathbb{Z}, +, \cdot$) -- коммутативное ассоциативное кольцо с 1
...
\begin{itemize}
    ...
    $x \neq \overline{0} = \{ 0 + nk | k \in \mathbb{Z}\} = <n> \Rightarrow f$ -- неинъективна
\end{itemize}

\textbf{Следствие} $p$ -- простое, $\mathbb{Z}/p\mathbb{Z}$ -- поле

\begin{center}
    \textbf{Доказательство}
\end{center}

Пусть $\overline{a} \neq \overline{0}$, т.е. $a \not\, \vdots \, p \Rightarrow (a,p) = 1$, т.е. $x \rightarrow \overline{a} \cdot x$ сюрьективно

то есть $\exists b \in \mathbb{Z} : \overline{a} \cdot \overline{b} = 1 \Rightarrow \overline{b} = \overline{a}^{-1}$, т.е. у $\overline{a}$ есть обратный $\Rightarrow \mathbb{Z}/p\mathbb{Z}$ -- поле
...
\textbf{Rem.} $\mathbb{Z}/0\mathbb{Z} = \mathbb{Z}$
    ...
    $\overline{a} \cdot \overline{x} = \overline{c}$ в $\mathbb{Z}/b\mathbb{Z}$

    $\exists!\overline{x_0} : (a,b) = 1$

    $ax \equiv c \Leftrightarrow x \equiv x_0$, т.е. $x = x_0 + bk, k \in \mathbb{Z}$
...
\begin{enumerate}
    \item $\exists x \in \mathbb{Z} : \begin{cases}
    ...
    \item Пусть $x_0$ -- такое, тогда все $x$, удовлетворяющие условию, это $\overline{x_0}$ в $\mathbb{Z}/mn\mathbb{Z}$
\end{enumerate}
...
\begin{enumerate}
    \item $x-a \, \vdots \, m$, т.е. $\begin{cases}
        x-a = my \\
        x-b = nz
    \end{cases}$
    ...
    \item В $\mathbb{Z}/m\mathbb{Z}$ $\overline{x} = \overline{a} = \overline{x_0}$

    В $\mathbb{Z}/n\mathbb{Z}$ $\overline{x} = \overline{b} = \overline{x_0}$
    ...
    \end{cases} \Leftrightarrow \begin{cases}
        ...
        x - x_0 \, \vdots \, n \\
        (a,b) = 1
    \end{cases} \Leftrightarrow x - x_0 \, \vdots \, mn \Leftrightarrow \overline{x} = \overline{x_0}$ в $\mathbb{Z}/mn\mathbb{Z}$
\end{enumerate}
...
$\begin{cases}
    ...
    f(x) = e^x \\
    e^{x+y} = e^x \cdot e^y
\end{cases} \Rightarrow f$ -- гомоморфизм, но точнее это $f : (\mathbb{R}, +) \rightarrow (\mathbb{R}^*, \cdot)$, но не изоморфизм
...
Все элементы $G$ разбились на циклы длины $k \Rightarrow n \, \vdots \, k$
...
Или $a \in Z;\ a^p - a \, \vdots \, p \Leftrightarrow a(a^{p-1}-1) \, \vdots \, p \Leftrightarrow \left[ \begin{gathered}
    a \, \vdots \, p \\
    a^{p-1} - 1 \, \vdots \, p
\end{gathered} \right.$
\vspace{5mm}
...
\begin{enumerate}
    ...
    Корректность: $q : \overline{p} = \overline{q} \Rightarrow p - q \, \vdots \, n$
...
Рассмотрим $n \in \mathbb{N}$. $n - 1 = p_1^{a_1} \ldots p_5^{a_5}$, т.е. $n = \prod p_i^{a_i} + 1$. Простое ли $n$?
...
Пусть $(x^k)^l = 1;\ x^{kl} = 1 \Leftrightarrow kl \, \vdots \, ord(x) \Leftrightarrow dk_1l \, \vdots \, dn_1 \Leftrightarrow k_1b \, \vdots \, n_1 \Leftrightarrow l \, \vdots \, n_1$, т.е. $n_1 = min(l)$
...
$a \in Z;\ a \not\, \vdots \, p$. Рассмотрим в $Z/pZ$ последовательность $\overline{a}^l, \overline{a}^{2l} \ldots \overline{a}^{2^ml} = a^{p-1} = 1$
...
Знаем: $\exists a \not\, \vdots \, p$. Рабин-Миллер для $a$ не выполнен $\Rightarrow p$ -- составные
...
\textbf{Th.} Теорема Безу: остаток от деления $f$ на $(x - a)$ равен $f(a)$. В частности $a$ -- корень $f \Leftrightarrow f \, \vdots \, x - a$
...
$a$ -- корень кратности $l$, если $V_{(x - a)}(f) = l$, т.е. $f \, \vdots \, (x - a)^l,\ f \not \, \vdots \, (x - a)^{l+1}$, $f = (x - a)^l \tilde{f} : \tilde{f}(a) \neq 0$
...
$f' = px^{p-1} = 0$ -- корень кратности $\infty$ $0 \, \vdots \, x^n \forall n$
...
\begin{enumerate}
    ...
    Т.е. $\exists s \in Z : \frac{2 \pi (k - k')}{n} = 2\pi s \Leftrightarrow k - k' \, \vdots \, n \Leftrightarrow \overline{k} = \overline{k'}$ в $Z/nZ$
