...
Что можно сказать про $(\mathbb{R}, \geq_\mathbb{R})$? Можно построить биекцию $x \mapsto x + 1$ -- это автоморфизм
...
\begin{Remark}{}
    ...
    Смотрим на $\mathbb{Z}, \mathbb{Q}$. Пусть $\exists h : \mathbb{Z} \to \mathbb{Q}$ 

    Рассмотрим двойку и тройку

    $\not\exists x \in \mathbb{Z} : 2 < x < 3$
...
\begin{theo}{}
    ...
    Это значит, что $X \cong \mathbb{Q}$
\end{theo}

\begin{Proof}{}

Возьмем $n$ точек из $X$ и $n$ точек из $\mathbb{Q}$. Построим между ними изоморфизм

Теперь нам нужен изоморфизм из $n + 1$ отрезков из $X$ в $n + 1$ отрезок множества $\mathbb{Q}$. Далее идем рекурсивно
...
\begin{Exercise}{}
    Попробуйте придумать явный изоморфизм между $\mathbb{Q}$ и $\mathbb{Q} \cap (0, 1)$
\end{Exercise}

\begin{Remark}{}
    $x \to x + 1$ -- автоморфизм $\mathbb{Z}$

    $h(\N) \neq \N$

    Пусть есть изоморфизм $g(\mathbb{Z}) \to \N$

    Применим прошлую функцию и получим $h(g(\mathbb{Z})) \to h(\N)$

    Но $g(h(\mathbb{Z})) = g(\mathbb{Z}) = \N$, а $h(\N) \neq \N$
\end{Remark}
...
\begin{rem}{Необоснованная индукция}
...
$\langle a_1, b_1 \rangle < \langle a_2, b_2 \rangle \Leftrightarrow \left[ \begin{gathered}
    a_1 < a_2 \\
    \begin{cases}
        a_1 = a_2 \\
        b_1 < b_2
...
$A_1 := \{ a | \langle a, b \rangle \in A \}$

$A_1 \subset \N \Rightarrow \exists a_1$ -- наименьший элемент $A_1$

$B_1 := \{b | \langle a_1, b \rangle \in A\}$

$B_1 \subset \N \Rightarrow \exists b_1$ -- наименьший элемент $B_1$

Иными словами, в $A$ есть элемент $\langle a_1, b_1 \rangle$. Про него мы знаем, что все остальные элементы из $A$ либо больше его по первой координате, либо равны с ним по первой и больше по второй

Значит $\langle a_1, b_1 \rangle$ -- наименьший, а тогда $A$ -- фундированное
...
$\langle a_1 \ldots a_n \ldots \rangle \setminus \langle b_1 \ldots b_n \ldots \rangle$ -- бесконечное количество
